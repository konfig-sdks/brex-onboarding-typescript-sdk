...
import { readableStreamToString, BrexOnboardingError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BrexOnboardingError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BrexOnboardingError(e, e.response?.data)
                }
                if (innerError instanceof BrexOnboardingError) {
                    // Got "BrexOnboardingError" from the above try block
                    throw innerError;
                }
